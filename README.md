...
Python recsys backend: **retrieve** a candidate set (latent two-tower, item-item CF, popularity), **rank** those candidates with user–item features, **serve** `GET /recommend`. Offline eval is recall@k, NDCG@k, and MRR@k on a temporal split; coverage@k is the popularity-bias check. Cold start, train/valid leakage, view-as-positive labels, and inverted ranking are guarded in pytest, not just described.
...
**Weak labels.** A valid-period `view` is not a held-out positive. Tests walk view-only pairs in the valid slice and assert they are absent from `held_out_positives`. Treating scroll events as visits would inflate recall/NDCG/MRR.
...
src/savor/
  ...
  evaluate.py    recall@k, NDCG@k, MRR@k, coverage
  metrics.py     metric definitions (first-hit MRR, truncated IDCG)
